<x-guest-layout>
    <form method="POST" action="{{ route('login') }}">
        ...
        <div class="flex items-center justify-center mt-4 text-sm text-gray-600 dark:text-gray-400">
            {{ __('¿No tienes cuenta?') }}
            <a class="underline ms-1 text-indigo-600 dark:text-indigo-400 hover:underline" href="{{ route('register') }}">
                {{ __('Regístrate') }}
            </a>
        </div>
    </form>
